...
try {
    // Buscar la categoría "Postres de crema o frías"
    $query = "SELECT id_catg, nombr_catg FROM categorias WHERE nombr_catg = 'Postres de crema o frías'";
    $stmt = $conn->prepare($query);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows === 0) {
        echo "<!-- No se encontró la categoría 'Postres de crema o frías' -->";
        exit;
    }
...
<rdf:RDF
   ...
   <!-- Categoría "Postres de crema o frías" -->
