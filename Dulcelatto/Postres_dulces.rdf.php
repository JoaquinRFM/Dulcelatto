...
try {
    // Buscar la categoría "Postres dulces"
    $query = "SELECT id_catg, nombr_catg FROM categorias WHERE nombr_catg = 'Postres dulces'";
    $stmt = $conn->prepare($query);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows === 0) {
        echo "<!-- No se encontró la categoría 'Postres dulces' -->";
        exit;
    }
