...
try {
    // Buscar la categoría "Galletas y Bizcochos"
    $query = "SELECT id_catg, nombr_catg FROM categorias WHERE nombr_catg = 'Galletas y Bizcochos'";
    $stmt = $conn->prepare($query);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows === 0) {
        echo "<!-- No se encontró la categoría 'Galletas y Bizcochos' -->";
        exit;
    }
